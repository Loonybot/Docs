...
It supports a wide range of devices, including goBilda Pinpoint Odometry Computers and
...
as a whole.
It can be helpful when you're having a debate as to whether bad behavior is a hardware bug or
a software bug.
...
**Test Bench** is simple to integrate. The code is a single `.java` file without non-SDK
dependencies, so you can simply copy it into your code. Alternatively, include it as a library.

**Test Bench** has support for a wide range of devices, but if it's missing your
favorite device, please consider adding support. 
**Test Bench** is modular and simple to extend, and we're happy to consider Pull Requests.
